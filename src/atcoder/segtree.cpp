#include <pybind11/functional.h>
#include <pybind11/pybind11.h>
#include <pybind11/stl.h>

#include <atcoder/internal_bit>

// 本家は非型テンプレートとして関数ポインタが必要なのでそのままバインドできない。
// std::functionを使って再実装する。
// std::vector<py::object> を使うのであまり速くはなさそう・・・

namespace py = pybind11;
using S = py::object;

namespace atcoder {

struct segtree {
  public:
    explicit segtree(std::function<S(S, S)> op, std::function<S()> e, int n)
        : segtree(op, e, std::vector<S>(n, e())) {}
    explicit segtree(const std::function<S(S, S)>& op,
                     const std::function<S()>& e,
                     const std::vector<S>& v)
        : _n(int(v.size())), op(op), e(e) {
        log = internal::ceil_pow2(_n);
        size = 1 << log;
        d = std::vector<S>(2 * size, e());
        for (int i = 0; i < _n; i++) d[size + i] = v[i];
        for (int i = size - 1; i >= 1; i--) {
            update(i);
        }
    }

    void set(int p, S x) {
        assert(0 <= p && p < _n);
        p += size;
        d[p] = x;
        for (int i = 1; i <= log; i++) update(p >> i);
    }

    S get(int p) const {
        assert(0 <= p && p < _n);
        return d[p + size];
    }

    S prod(int l, int r) const {
        assert(0 <= l && l <= r && r <= _n);
        S sml = e(), smr = e();
        l += size;
        r += size;

        while (l < r) {
            if (l & 1) sml = op(sml, d[l++]);
            if (r & 1) smr = op(d[--r], smr);
            l >>= 1;
            r >>= 1;
        }
        return op(sml, smr);
    }

    S all_prod() const { return d[1]; }

    int max_right(int l, const std::function<bool(S)>& f) const {
        assert(0 <= l && l <= _n);
        assert(f(e()));
        if (l == _n) return _n;
        l += size;
        S sm = e();
        do {
            while (l % 2 == 0) l >>= 1;
            if (!f(op(sm, d[l]))) {
                while (l < size) {
                    l = (2 * l);
                    if (f(op(sm, d[l]))) {
                        sm = op(sm, d[l]);
                        l++;
                    }
                }
                return l - size;
            }
            sm = op(sm, d[l]);
            l++;
        } while ((l & -l) != l);
        return _n;
    }

    int min_left(int r, const std::function<bool(S)>& f) const {
        assert(0 <= r && r <= _n);
        assert(f(e()));
        if (r == 0) return 0;
        r += size;
        S sm = e();
        do {
            r--;
            while (r > 1 && (r % 2)) r >>= 1;
            if (!f(op(d[r], sm))) {
                while (r < size) {
                    r = (2 * r + 1);
                    if (f(op(d[r], sm))) {
                        sm = op(d[r], sm);
                        r--;
                    }
                }
                return r + 1 - size;
            }
            sm = op(d[r], sm);
        } while ((r & -r) != r);
        return 0;
    }

  private:
    int _n, size, log;
    std::function<S(S, S)> op;
    std::function<S()> e;
    std::vector<S> d;

    void update(int k) { d[k] = op(d[2 * k], d[2 * k + 1]); }
};

}  // namespace atcoder

PYBIND11_MODULE(segtree, m) {
    m.doc() = "atcoder/segtree";
    py::class_<atcoder::segtree>(m, "segtree")
        .def(py::init<const std::function<S(S, S)>&, const std::function<S()>&,
                      int>(),
             py::arg("op"), py::arg("e"), py::arg("n") = 0)
        .def(py::init<const std::function<S(S, S)>&, const std::function<S()>&,
                      const std::vector<S>&>(),
             py::arg("op"), py::arg("e"), py::arg("v"))
        .def("set", &atcoder::segtree::set, py::arg("p"), py::arg("x"))
        .def("get", &atcoder::segtree::get, py::arg("p"))
        .def("prod", &atcoder::segtree::prod, py::arg("l"), py::arg("r"))
        .def("all_prod", &atcoder::segtree::all_prod)
        .def("max_right", &atcoder::segtree::max_right, py::arg("l"),
             py::arg("f"))
        .def("min_left", &atcoder::segtree::min_left, py::arg("r"),
             py::arg("f"))
        /* Workaround to make it look as if it is a Generic class.*/
        .def("__class_getitem__", [](py::object) -> void {});
}
